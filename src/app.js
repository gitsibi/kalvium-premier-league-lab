...
function sortByNameAwardxCountry(awardName, noOfTimes, country) {
  const filteredPlayers = players.filter(player => player.country === country && player.awards.filter(award => award.name === awardName).length === noOfTimes);
...
function sortByNameOlderThan(age) {
  ...
  olderPlayers.forEach(player => player.awards.sort((a, b) => b.year - a.year));
  return olderPlayers.sort((a, b) => a.name.localeCompare(b.name));
}
